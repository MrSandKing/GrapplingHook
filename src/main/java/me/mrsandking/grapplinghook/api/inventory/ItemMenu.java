...
public class ItemMenu {
    ...
    public void open(Player player) {
        if (!ItemMenuListener.getInstance().isRegistered(GrapplingHookMain.getInstance())) {
            ItemMenuListener.getInstance().register(GrapplingHookMain.getInstance());
        }
            ...
            if (slot >= 0 && slot < size.getSize() && items[slot] != null) {
                ...
                if (itemClickEvent.willUpdate()) {
                    ...
                    if (itemClickEvent.willClose()) {
                        ...
                        Bukkit.getScheduler().scheduleSyncDelayedTask(GrapplingHookMain.getInstance(),
